...
def _draw_circles(src, points, line=False):
    if src.ndim == 2:
    ...
    for i, (y, x) in enumerate(points):
        ...
        if line:
            cv2.line(_drawing, points[i-1][::-1],
                     points[i][::-1], (0, 255, 0), 1)
...
def _put_points(src, lines=False):
    ...
    def draw_circle(event, x, y, flags, param):
        ...
        _draw_circles(src, points, lines)
...
def active_contour(src, window_size=7, alpha=0.4, beta=0.5, thresh=0):
    ...
    grids = np.concatenate([np.array(y).reshape(
        *magnitude.shape, 1), np.array(x).reshape(*magnitude.shape, 1)], axis=2)
    ...
    while diff > thresh:
        ...
        for i, (y, x) in enumerate(points.copy()):
            ...
            elastic = np.sum((points[ni]-(2*G)+points[pi])*beta, axis=2)

            vals = max_diff+smooth+elastic
            _draw_circles(magnitude.copy().astype("uint8"), points, True)
            dir = np.array(np.unravel_index(
                [np.argmin(vals)], M.shape)).reshape(-1)
