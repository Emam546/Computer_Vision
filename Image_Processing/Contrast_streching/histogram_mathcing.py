...
def adaptive_hist_equ(src, kernel=(8, 8)):
    ...
    new_image = cv2.copyMakeBorder(
        src, pady, padx, pady, padx, cv2.BORDER_REPLICATE)
    undefined_image = np.zeros_like(src, "uint8")
    h, w = new_image.shape
            ...
            for j in range(y-pady, y+pady):
                for i in range(x-padx, x+padx):
                    if new_image[y, x] >= new_image[j, i]:
                        rank += 1
            undefined_image[iy, ix] = round(255*(rank/total_region))
            # print(rank,x)
   # print(total_region)
...
def _main():
    ...
    img = cv2.imread("./stretch_original.jpg", 0)
